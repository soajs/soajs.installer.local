...
let dockerModule = {
	...
	install: (args, callback) => {
		let execPath = path.normalize(process.env.PWD + "/../soajs.installer.local/libexec/bin/FILES/DOCKER");
		...
		if (process.env.PLATFORM === 'Darwin') {
			...
			exec("sudo docker ps", (err, data) => {
				if(err && !err.toString().includes("Cannot connect to the Docker daemon at unix:///var/run/docker.sock. Is the docker daemon running?")){
					...
					command = path.normalize(process.env.PWD + "/../soajs.installer.local/libexec/bin/FILES/DOCKER/docker-linux-start.sh");
